...
import pre_processing.labeling
...
def make_segments(filepath, activity):
    df_activity = pre_processing.labeling.extract_one_activity(filepath, activity, save_file=False)
    peaks,_ = find_mag_peaks(df_activity, 1000, 900)
...
def make_start_stop_segmentation_file(filepath_arm, filepath_back, filepath_start_stop_labels, filepath_folder):
    ...
    output_path = f"{filepath_folder}/start_stop_segments.csv"
